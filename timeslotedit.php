...
*/
 ...
 if (isset($_REQUEST['addslot']) )
  ...
  $result = mysql_query($query) or die(mysql_error());
  $counter=0;
  if(mysql_num_rows($result)==0) echo "<tr><td colspan='7'>No timeslots available</td></tr>";
  while($row = mysql_fetch_array($result, MYSQL_ASSOC))
  {
  ...
  if (isset($_GET['action']) && isset($_GET['id']))
  {
    switch($_GET['action'])
    {
      case 1:
      {
        ...
        $counter2 = 0;  
        if(mysql_num_rows($result)==0) echo "<tr><td colspan='2'>No participants found</td></tr>";
        while($row = mysql_fetch_array($result, MYSQL_ASSOC))
        {        
...
?>
  ...
  <tr>
    ...
    <td>Date:<br><small>(mm/dd/yyyy)</small></td><td>
      <input type="text" class='textInput' name="edate" maxlength="10" value="<? if ($rememberFormValues!=0 && isset($_POST['edate'])){echo $original_date;} ?>" >
    </td>
    <td>
