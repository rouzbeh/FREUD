...
if(isset($_SESSION['permission']) && ($_SESSION['permission']=="admin" || $_SESSION['permission']=="researcher")){
  ...
  {    
    ...
    {     
      ...
      {
        $_REQUEST['experiment_id'] = addslashes($_REQUEST['experiment_id']);
        $_REQUEST['edate'] = addslashes($_REQUEST['edate']);
      }
      $original_date = $_REQUEST['edate'];
...
/*        $etime1=$_REQUEST[$hour];
        if ($_REQUEST[$daypart]=="pm") $etime1+=12;
        if ($etime1==24) $etime1=0;
        $etime1=$etime1.":".$_REQUEST[$minute].":00";
*/
...
$query = "SELECT timeslot.*, experiment.title, experiment.researcher_email, 
  ...
  $result = mysql_query($query) or die(mysql_error());
  $counter=0;
  if(mysql_num_rows($result)==0) echo "<tr><td colspan='5'>No available timeslots</td></tr>";
  while($row = mysql_fetch_array($result, MYSQL_ASSOC))
  {
  ...
  if (isset($_GET['action']) && isset($_GET['id']))
  {
    switch($_GET['action'])
    {
      case 1:
      {
        ...
        $counter2 = 0;  
        if(mysql_num_rows($result)==0) echo "<tr><td colspan='2'>No participants</td></tr>";
  ...
  function printTimeslot($id, $idmax)
  {
    ...
    for ($ii=1; $ii<=12;$ii++)
    {
      ...
      if ($rememberFormValues!=0 && isset($_REQUEST[$hour]) && $_REQUEST[$hour]==$ii){echo " selected=\"yes\" ";}
      echo ">".$ii."</option>\n";
    }
    ...
    for ($ii=0; $ii<60;$ii+=5)
    {
	  if ($ii<10) $ii="0".$ii;
      echo "        <option value=\"".$ii."\"";
      if ($rememberFormValues!=0 && isset($_REQUEST[$minute]) && $_REQUEST[$minute]==$ii){echo " selected=\"yes\" ";}
      echo ">".$ii."</option>\n";
    }

    echo "   </select>\n";
    echo "    <select name=\"".$daypart."\">\n";
    echo "      <option value=\"am\""; if ($rememberFormValues!=0 && isset($_REQUEST[$daypart]) && $_REQUEST[$daypart]=="am"){echo " selected=\"yes\" ";} echo ">am</option>\n";
    echo "      <option value=\"pm\""; if ($rememberFormValues!=0 && isset($_REQUEST[$daypart]) && $_REQUEST[$daypart]=="pm"){echo " selected=\"yes\" ";} echo ">pm</option>\n";
    echo "    </select>\n";
    echo "  </td>\n";
    ...
    {
      echo "        <option value=\"".$ii."\"";
      if ($rememberFormValues!=0 && isset($_REQUEST[$capacity]) && $_REQUEST[$capacity]==$ii){echo " selected=\"yes\" ";}
      echo ">".$ii."</option>\n";
    }
    echo "    </select>\n";
    echo "  </td>\n";
...
<table style="width:auto">  
  ...
  <tr>
    <td>Date:<br><small>(mm/dd/yyyy)</small></td><td>    
      <input type="text" class='textInput' name="edate" maxlength="10" value="<? if ($rememberFormValues!=0 && isset($_REQUEST['edate'])){echo $original_date;} ?>" >
    </td>
    <td>
